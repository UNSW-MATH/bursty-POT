...
*Proof of Theorem:* 	Due to assumption \eqref{assumptionJs}, it follows
	\begin{align*}
	P\left(\frac{M(c)-d(c)}{a(c)} \leq x\right) \rightarrow F(x) \text{ as } c \rightarrow \infty,
	\end{align*}
	where $x$ is any value from the support of $F$. Then it follows,
	\begin{align*}
    ...
    \end{align}
	 with $p(\ell_c):= \overline{F}(\ell_c)$. Since $\tau(\ell_c) \sim Geo(p(\ell_c))$ and \eqref{eq:lplconv}
	...
	\end{align*}
	 with parameter $\lambda:=-\log F_A(x)$. Due to
	\begin{align*}
	S(c):= \sum_{i=1}^{\lfloor c \rfloor} \frac{W_i}{b(c)} \Rightarrow D \ \text{ as } c \rightarrow \infty,
	\end{align*}
    it follows with Gnedenkos transfer theorem (see @gnedenko1983limit), that
	\begin{align}
	...
	\end{align}
    where $\Phi_D$ resp. $F_E$ denotes Laplace transform of $D$ resp. the distribution function of $E$. Hence, $Z$ is Mittag-Leffler distributed with parameter $\lambda^{-1/\beta}$. Since
	\begin{align*}
	...
	\end{align*}
	the second factor converges to $\lambda^{1/\beta}$, it follows that
...
tbl_ML_estim %>% filter(method=="logMom" | method=="ML")%>%
  ...
  scale_color_manual(values=c("red", "blue"),name = "Method", labels = c("log-Moment", "MLE"))
