...
and utilize a result by @MeerschaertStoev08. 
The running maximum process is defined as 
$$
M_J(t) := J_1 \vee \ldots \vee J_{\lfloor t \rfloor},
$$
and as we assume the $J_k$ have a continuous distribution, there exist 
norming functions $a(c)$ and $d(c)$ such that 
$$
\frac{M_J(t) - d(t)}{a(t)} \stackrel{d}{\longrightarrow} A, \quad t \to \infty
$$
where $A$ is a random variable with a generalized extreme value distribution. 
The underlying CTRM is then defined via 
$$
X(t) = M_J(N(t)), \quad t \ge 0
$$
...
convergence 
$$
\frac{V(ct) - d(\tilde b(c))}{a(\tilde b(c))} \stackrel{d}{\longrightarrow} 
A(E(t)), \quad t > 0.
$$
where: 

* $A(t)$ is the extremal process such that $A(1) \stackrel{d}{=} A$, 
* $E(t) = \inf\{u: D(u) > t\}$ is an inverse subordinator, 
  associated with the Levy process $D(t)$ that satisfies 
  $D(1) \stackrel{d}{=} D$,
* $\tilde b(c)$ is a regularly varying norming function which is
  _inverse_ to $b(c)$: $b(\tilde b(c)) \sim c \sim \tilde b(b(c))$. 
