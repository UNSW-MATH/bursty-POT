...
_Proof of Theorem:_
We interpret the threshold crossing time as the hitting time of the 
...
convergence 
$$
\frac{M(ct) - d(\tilde b(c))}{a(\tilde b(c))} \stackrel{d}{\longrightarrow} 
M(t), \quad t > 0.
$$
where $M(t)$ is a time-changed ("subordinated") extremal process, and where 
...
= \mathbf P[M(ct) \le \ell]
= \ldots
$$
where we have substituted $c = b(1/p)$, 
$$
\ldots
= \mathbf P\left[ \frac{M(ct) - d(\tilde b(c))}{a(\tilde b(c))} 
\le \frac{\ell - d(\tilde b(c))}{a(\tilde b(c))} \right]
\longrightarrow \mathbf P\left[ M(t) \le \ell^* \right]
= \ldots
$$
Defining the hitting time of level $\ell^*$ by $M(t)$ as 
$$
\xi_{\ell^*} := \inf\{t: M(t) > \ell^*\}
$$
we then have 
$$
\ldots = \mathbf P[\xi_{\ell^*} > t] 
= \mathbf P[(-\log F(\ell^*))^{-1/\beta} X^{1/\beta} D]
$$
by [Proposition 4.2, @MeerschaertStoev08]. Using [Theorem 19.1, @Haubold11], 
we see that $X^{1/\beta} D \sim {\rm ML}(\beta, 1)$, concluding the proof.


# Statistics of the Mittag-Leffler distribution
