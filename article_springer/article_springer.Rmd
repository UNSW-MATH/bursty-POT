...
In the classical POT approach a GPD distribution is fitted to the data. Due to the Theorem in the last Section we know that for high threshholds the waiting times between the exceedances are Mittag-Leffler distributed in case of heavy tailed waiting times. Hence, we discuss in this section first parameter estimation for Mittag-Leffler distributions.  

The first method proposed for parameter estimation of the Mittag-Leffler distribution was the fractional moment estimator by [@kozubowski2001]. One drawback of this method is that constant priors for the tail parameter are needed for the calculation of the estimates. [@Cahoy2010] proposed an estimator based on the log-Moments which doesn't need prior knowledge. Furthermore, they showed in a simulation study that the log-Moments estimator outclasses the fractional moment estimator regarding bias and root mean squared error (RMSE). Due to the form of the density function there exists no closed form for the Maximum Likelihood estimator (MLE). In the RPackage `MittagLeffleR` Maximum Likelihood estimation is implemented via numerical optimization. The MLE outperforms the log-moment estimator regarding bias and RMSE for big enough sample sizes, but is extremely computational intensive and the differences in bias and RMSE between the two methods are very small. In Figure \ref{fig:MSE} one can see that both methods work quite well even for very low sample sizes.    
Since the Mittag-Leffler distribution is heavy-tailed, many researchers would intuitively give the highest importance to the tail behaviour of the distribution. Of course, one can also use established tail exponent estimators such as the Hill estimator. However, these methods are less statistically efficient since they only use a portion of the information contained in the data, as was already mentionded by [@kozubowski2001]. Addtionally they need a specification of a tuning parameter. In XX one can see a Hill plot for differet k's.  
...
```{r LRT power, echo=FALSE, fig.cap="Type One error for bootstrapped LRT for different sample sizes", out.width = '100%'}
knitr::include_graphics("Plots_LRT_boot.pdf")
```


```{r LRT type one error, echo=FALSE, fig.cap="Type One error for bootstrapped LRT for different sample sizes", out.width = '100%'}
knitr::include_graphics("Plots_LRT_boot_typeOneError.pdf")
```
