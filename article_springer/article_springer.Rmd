...
Code used for the analysis in this article has been organized into 
an R package `CTRM` (<https://github.com/UNSW-MATH/CTRM>).
The source code for this manuscript is available online at <https://github.com/UNSW-MATH/bursty-POT>. 
...
flare_mags <- flares %>% ctrm() %>% thin(k=500) %>% coredata()
...
(bottom-center panel) one hardly identifies $0.8$ as a stable value, and 
thus as a clear candidate for a tail parameter estimate.

QQ-estimators, and the closely related Hill estimator, are hence not 
suitable to detect a heavy-tailed Mittag-Leffler distribution. Indeed, 
...
flares_ctrm %>% interarrival() %>% mlqqplot(log = 'xy', tail = 0.8, main = "Mittag-Leffler QQ Plot")
```
...
par(mfrow=c(1,2))
MLestimates(ctrm = thin_sim_ctrm, tail = params$tail, scale = n^(1/params$tail))
```
