...
The classical POT approach is based on the fact, that exceedances above a high threshhold are assymptotically GPD distributed, hence a GPD distribution is fitted to the exceedances above a high enogh threshhold. Due to the Theorem in the last Section we know that for high threshholds the times between the exceedances are assymptotically Mittag-Leffler distributed in case of heavy tailed waiting times between the observations. Hence, before we talk about inference for the exceedances in the next section, we want to have a look at inference for Mittag-Leffler distributions in the following. 

The first method proposed for the estimation of the Mittag-Leffler distribution parameters was the fractional moment estimator by [@kozubowski2001]. Unlike the first moments, the fractional moments of order $p$ for $p<\beta$ exist and can get calculated. One drawback of this method is that constant priors for the tail parameter are needed for the calculation of the estimates. [@Cahoy2010] proposed an estimator based on the log-Moments which doesn't need prior knowledge. Furthermore, they showed in a simulation study that the log-Moment outclasses the fractional moment estimator regarding bias and root mean squared error (RMSE).  
Due to the form of the density function of a Mittag-Leffler distribution there exists no closed form for the Maximum Likelihood estimator (MLE). In the RPackage `MittagLeffleR` Maximum Likelihood estimation is implemented via numerical optimization. The MLE outperforms the log-moment estimator regarding bias and RMSE for big enough sample sizes, but is extremely computational intensive and the differences in bias and RMSE between the two methods are very small. In Figure \ref{fig:MSE}, one can see that both methods work quite well even for very low sample sizes.    
...
\end{align*}
However, these methods are less statistically efficient since they only use a portion of the information contained in the data, as was already mentioned by [@kozubowski2001]. Additionally, they need a specification of a tuning parameter and the hill estimator only performs well for distributions close to Pareto (see @Resnick97). In Figure \ref{fig:Hillplots} one can see different Hill plots for Mittag-Leffler simulated data with differrent sample sizes and tails. It would be really hard to deduce the correct tail parameter estimates from this plots. 
...
independent waiting time and magnitude pairs $(W_k, J_k)$ (see upper panel in 
Figure \ref{fig:thresholdedBursty}) for three different situations: stable waiting times, Pareto waitings times and Exponential waiting times. In order to have exact 
...
show the diagnostic plots for a minimum threshold chosen at the 200th order statistic. 
There is some residual autocorrelation for the sequence of threshold exceedance times that is not accounted for by the CTRE model. The fit with a Mittag-Leffler distribution ($\beta = 0.8$) is good, though there are signs that the power-law tail tapers off for very large inter-threshold crossing times. There is no apparent dependence between threshold exceedance times and event magnitudes seen in the copula plot. 
