...
$$
and since the $J_k$ lie in the domain of attraction of a max-stable distribution $F$ (see \eqref{assumptionJs}), there exist 
...
4. Plot $k$ vs.\ $k^{1/\hat \beta} \hat \sigma_k$. Again, choose a region of *stability* and, and choose a representative $\hat \sigma_0$ from that region.
...
event magnitudes (subject to a mixing condition). The two approaches are fundamentally different: Clustering assumes that each event belongs to one particular (latent) group of events. For bursts, however, the aim is to identify an underlying scale-free pattern in the event 
dynamics, which is often characteristic of complex systems. It is an interesting open problem to develop criteria, based e.g. on measures of surprise [@Lee15], that guide an applied statistician in the choice between a clustering and a CTRE approach for a particular problem. 
In sum, the CTRE approach should not be seen as a substitute for the usual cluster approach, but rather as a complementary approach. 

Moreover, it was observed in @towe2020modelling that clusters of extreme events still exist after declustering. We believe it is possible to marry the clustering approach with our CTRE approach by considering CTREs based on MRPs with a _stationary_, rather than i.i.d., sequence of magnitudes, so that the times between clusters are asymptotically Mittag-Leffler distributed. More broadly speaking, the identification of clusters in the context of heavy-tailed inter-arrival times is an open problem. 
