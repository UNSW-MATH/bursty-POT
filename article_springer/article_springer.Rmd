...
abstract: |
  ...
  show how the Mittag-Leffler distribution has a memory which affects 
  the predictive distribution of the next extreme event. 
...
devtools::install_github("UNSW-MATH/CTRM")
library(CTRM)
```
...
In Section 7, we discuss the non-memorylessness property of the 
Mittag-Leffler distribution, and how it affects the predictive distribution 
for the next threshold crossing event. 
Section 8 concludes.
For all statistical computations we have used R [@R], and all code, data, as 
well as the source code for this manuscript has been organized into an R 
software package, available at <https://github.com/UNSW-MATH/CTRM>. 
...
In this setup, the $k$-th magnitude $J_k$ occurs at time
$T_k = W_1 + \ldots + W_k$. 
...
The top-right panel of Figure \ref{fig:QQ-plots}, for instance, shows the 
...
(see bottom-left panel of Figure \ref{fig:QQ-est}). There is no apparent interval of stability, 
...
the light-tailed ${\rm Exp}(1)$ distribution (compare the right two panels). 
In particular, if only few (e.g. $<100$) data are available, 
then the static QQ-estimator plots look interchangeable for the two 
distributions. 
...
thin_flares_ctrm <- flares %>% ctrm() %>% thin(k = 700)
par(mfrow=c(1,2))
MLestimates(thin_flares_ctrm, tail = 0.9, scale = 3 * 1E7)
```

The Theorem in Section \ref{sec:scaling} implies that for high 
...
flares_ctrm <- flares %>% ctrm() %>% thin(k = 200)
acf(flares_ctrm)
```
...
par(mfrow = c(2,2))
empcopula(flares_ctrm)
mlqqplot(flares_ctrm, log = 'xy', tail = 0.8, main = "Mittag-Leffler QQ Plot")
```

The MRP model is based on three main assumptions, which are repeated 
...
exceedance times that is not accounted for by the MRP model. 
...
tail as well as its streteched exponential (peaked) asymptotics for small 
times. 
...
When we introduced the MRP model, we assumed that all events are i.i.d. 
This assumption is likely sufficient but not necessary for our limit theorem to hold. 
Morevoer, any data below a (minimum) threshold $\ell_0$ is discarded in 
...
For some bursty time series, the current MRP model may be too rigid, 
