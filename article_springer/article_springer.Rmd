...
$\{(t^{(\ell)}_i, x^{(\ell)}_i), i \in I(\ell)\}$, with $x^{(\ell)}_i=x_i-\ell$, where $I(\ell):=\left\{i: x_i \le \ell \right\} \subseteq I$.
...
In case we would have data of a MRP $\{(W_i, J_i): i \in I\}$ as defined in Section \ref{sec:CTRE}, we could apply any tail (and scale) estimator such as e.g. Hill's estimator [@hill1975simple] to the $W_i$ and thus infer the distribution of inter-exceedance times. However, in real-world datasets, the MRP assumption is too strong for low thresholds. The unknown data generating process is likely to be more complex, with dependencies or mutliple data generating mechanisms at low thresholds, structures that possibly vanish for higher thresholds. Fitting only the IETs means that the iid assumption applies to exceedances and IETs only.  
